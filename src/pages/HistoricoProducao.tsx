...
const COLUMNS = SABORES.map((s) => ({ id: s.id, label: s.id, emoji: s.emoji }));
...
export default function HistoricoProducao() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ resource: "historico-producao", start, end });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 pb-8">
      <h1 className="text-lg font-bold text-brand-brown">🏭 Histórico de Produção</h1>
      <HistoricoTable
        rows={rows}
        columns={COLUMNS}
        loading={loading}
